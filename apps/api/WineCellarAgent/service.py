...
class WineCellarAnalysisService:
    ...
    @staticmethod
    async def analyze_cellar(db_url: str | None = None) -> WineCellarAnalysis:
        """
        Analyze the wine cellar and return structured recommendations
        
        Args:
            db_url: Optional custom database URL (defaults to DATABASE_RO_URL env var)
        
        Returns:
            WineCellarAnalysis: Structured analysis with recommendations and criticality levels
        """
        agent = WineCellarAgent(db_url)
